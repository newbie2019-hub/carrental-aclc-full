<template>
  <div>
    <v-container>
      <v-layout column>
        <h1 class="custom-primary-color">Account Settings</h1>
        <p class="">Please make sure that your informations entered are correct.</p>

        <v-row>
          <v-col cols="12" sm="12" md="11" lg="11" xl="8">
            <v-divider class="mt-7" />
            <h2 class="mt-10">Personal Info</h2>
            <v-form ref="form" v-model="valid" @submit.prevent="update" lazy-validation>
              <v-row dense class="mt-3">
                ...
                <v-col cols="11" sm="10" md="6" lg="6">
                  <v-text-field append-icon="mdi-card-bulleted" class="" hide-details="auto" label="Middle Name" :loading="isLoading" v-model="data.middle_name" required outlined dense></v-text-field>
                </v-col>
                <v-col cols="11" sm="10" md="6" lg="6">
                  <v-text-field
                    append-icon="mdi-card-bulleted"
                ...
                <v-col cols="11" sm="10" md="6" lg="6">
                  <v-select v-model="data.gender" :rules="required" :items="gender" label="Gender" hide-details="auto" outlined dense></v-select>
                </v-col>
                <v-col cols="11" sm="10" md="6" lg="6">
                  <v-text-field
              ...
              <v-row class="mt-3" align="center">
                <v-avatar color="grey darken-1" size="70" class="ml-3">
                  <img @click.prevent="showProfile" class="img-cover cursor-pointer" v-if="user.info.profile_img" :src="`http://127.0.0.1:8000/images/${this.user.info.profile_img}`" alt="John" />
              ...
              <v-layout v-if="user.info.role.role != 'Admin'" class="mt-7" column>
              ...
              <p class="">Please ensure that your login credentials are correct.</p>
              <v-row class="mt-3">
                <v-col cols="11" sm="10" md="6" lg="6">
                  <v-text-field
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      show() {
        this.$viewerApi({
          images: [`http://127.0.0.1:8000/images/${this.user.info.valid_id}`],
          options: {
            'inline': false,
            'button': true,
            'navbar': true,
